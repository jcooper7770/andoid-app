...
@app.route("/logger/delete", methods=["POST"])
def delete_practice():
    """
    Deletes the given practice
    """
    request_form = request.get_json()
    day = request_form.get("day")
    session['name'] = request_form.get("name", "bob")
    event = request_form.get("event", "trampoline")
    datetime_to_remove = datetime.datetime.strptime(day, "%m-%d-%Y")
    Practice.delete(datetime_to_remove, event=event)
    return  {"success": True}
...
def _save_trampoline_data(request):
    ...
    request_form = request.get_json()
    form_data = request_form.get('log', '')
    #username = request_form.get('name', None) or current_user()
    username = request_form.get('name', None) or session.get('name')
    event = request_form.get('event', None) or current_event()
    notes = request_form.get('notes', None)
    print(f"!!!!!!: {username} - {form_data}")
    set_current_event(event)
    set_current_user(username)
    set_current_athlete(username)
    logger.info(f"Username: {username}")
    routines = convert_form_data(form_data, event=event, notes=notes)
    logger.info(request_form.get('log', 'None').split('\r\n'))

    # Save new goal
    
    num_goals = request_form.get("goals_form")
    if num_goals:
        ...
        goal_string = request_form.get('goal_string', None)
        if goal_string:
        ...
        for key in request_form.keys():
            if key in ["goal_string", "goals_form"]:
                continue
            if key.startswith("delete_goal"):
                deleted_goal_num = int(key[11:])
    ...
    # Save airtime
    airtime = request_form.get("airtime_form")
    if airtime:
        all_airtimes = get_user_airtimes(current_user())
        airtime_str = request_form.get('airtime_string', '')
        try:
            airtime_str = f"{float(airtime_str):.2f}"
        except:
            pass
        # add in date field
        form_date_str = request_form.get('airtime-date', str(datetime.date.today()))
        ...
        for key in request_form.keys():
            if key in ["airtime_string", "airtime_form"]:
                continue
            if key.startswith("delete_airtime"):
                deleted_airtime_num = int(key[14:])
                delete_airtime_from_db(current_user(), all_airtimes[deleted_airtime_num])
                continue
   
    # Save the current practice
    form_date_str = request_form.get('date', str(datetime.date.today()))
    form_date = datetime.datetime.strptime(form_date_str, "%Y-%m-%d")
    ...
    replace_practice = request_form.get('replace', None) is not None
...
def trampoline_user_practices(username):
    ...
    if username == "_current_":
        ...
        return redirect(url_for('trampoline_user_practices', username=username, **request.args))
...
def trampoline_log2():
    ...
    for practice in user_practices:
        ...
        title_date = practice.date.strftime("%A %m/%d/%Y")
        title = f"{title_date} ({practice.event})"
        practice_dict['title'] = title
        practice_dict['event'] = practice.event;
        practice_dict['date'] = practice.date.strftime("%m-%d-%Y");
        practice_turns =[]
        for turn in practice.turns:
            practice_turn = {
                "note": turn.note,
                "skills": [skill.shorthand for skill in turn.skills],
                "flips": turn.total_flips,
                "difficulty": turn.difficulty,
            }
            practice_turns.append(practice_turn)
        practice_dict['turns'] = practice_turns

        all_practices.append(practice_dict)
    ...
    airtimes = get_user_airtimes(current_user())
    airtimes = [
        [x for x in a] for a in airtimes
    ]
    return_val = dict(
        template="trampoline/trampoline.html",
        #body=body,
...
def login2():
    ...
    if athlete.first_login:
        ...
        return {"success": True, "user": username, 'first_login': True}
    return {"success": True, "user": username, 'first_login': False}
